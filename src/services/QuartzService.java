...
public class QuartzService {
	...
	public void doProcess() {
		...
		try {
			...
			JobDetail job1 = newJob(WeeklySuspension.class).withIdentity("job1", "group1").build();
			JobDetail job2 = newJob(PaymentNotification.class).withIdentity("job2", "group1")
					.build();

			JobDetail job3 = newJob(ChasePayment.class).withIdentity("job3", "group1").build();
	...
	public void setupNewJobAndTrigger(String orderWindowId, Date startTime)
			throws SchedulerException {
		Scheduler sched = StdSchedulerFactory.getDefaultScheduler();
		JobDetail job1 = newJob(SendNotification.class).withIdentity(orderWindowId, "group2")
				.build();
		job1.getJobDataMap().put(SendNotification.ORDERPERIOD_ID, orderWindowId);
		SimpleTrigger trigger1 = (SimpleTrigger) newTrigger().withIdentity(orderWindowId, "group2")
	...
	public void setupNewJobAndTriggerForJobs(String jobType, int day, int hour, int min)
			throws Exception {
		Scheduler sched = StdSchedulerFactory.getDefaultScheduler();
		...
		Trigger newTrigger = tb.withSchedule(weeklyOnDayAndHourAndMinute(day, hour, min)
				.inTimeZone(TimeZone.getTimeZone("GMT+8:00"))).build();
